...
class StockModel(nn.Module):
    def __init__(self):
        ...
        self.attentionweight = nn.Linear(article_number * 768, article_number, bias= False)

        self.LSTMdecoder = nn.LSTM(768 + 4, hidden_dim, 2, batch_first=True, bidirectional=True)
    ...
    def forward(self, words, stock_info):
        ...
        pt_outputs = torch.bmm(pt_outputs.permute(0, 2, 1), attention_weight.unsqueeze(2))

        pt_outputs = torch.cat((pt_outputs.squeeze(), stock_info), dim=-1) # (N, 768 + 4)
